...
def main():
    ...
    KF_capacity[0,0] = 2300*link_lane_number
    ...
    try:    
        with open(address,'r') as data_csv:
                ...
                with sqlite3.connect(db_file) as conn:
                    ...
                    if ((round(t,1) % time_step) == 0): 
                        ...
                        if (round(t,1) == start_time):
                            [mean_speeds, max_speeds, probe_traffic_states] = add_data_to_PTS_table_time_space_diagram_method(conn, round(t,1), start_time, boundaries, cell_number,cell_length)  ## spatial buffer is 5 ft                    
                            ...
                            InputFlowRate = int(flow_measurement[0])/penetration_rate
                            ...
                            InputFlowRate_mea = int(flow_measurement[0])/penetration_rate
                            ...
                            #InputFlowRate = InputFlowRate_mea
                            ...
                            if ((mean_speeds.mean() != -1) and (max_speeds.mean() != -1)): 
                                ...
                                if (sum(low_speed_cell) >= 1): ### under congestion condition
                                    ...
                                    if ( Q_pre > jam_density/((1.0/KF_ffs[i,0])+(1.0/KF_bws[i,0])) ):
                                        Q_mea= jam_density/((1.0/KF_ffs[i,0])+(1.0/KF_bws[i,0]))
                                    else:
                                        Q_mea = Q_pre
                                    ...
                                    if (small_headway >0):
                                        ...
                                        if (swspeed_pre < KF_capacity[i,0]/(jam_density-(KF_capacity[i,0]/KF_ffs[i,0] ))  ):
                                            swspeed_mea = KF_capacity[i,0]/(jam_density-(KF_capacity[i,0]/KF_ffs[i,0] ))
                                        else:
                                            swspeed_mea = swspeed_pre
                            ...
                            for j in range(cell_number+1):
                                if (j == 0):    
                                    ...
                                    KF_flow[i][j] = min(KF_occ[i-1][j-1], KF_capacity[i][0]*time_step/3600, (KF_ffs[i][0]*1.0/KF_bws[i][0])*(jam_density*cell_length/5280-KF_occ[i-1][j-1]))
            ...
            try:
                occ_file = save_to+"\\KF_occ_3_20.csv"
                np.savetxt(occ_file, KF_occ,delimiter=',')
                occ_file_mea_speed = save_to+"\\KF_occ_mea_via_speed_3_20.csv"
                np.savetxt(occ_file_mea_speed, mea_occ_via_speed,delimiter=',')
                occ_file_predict_CTM = save_to+"\\KF_occ_pre_via_CTM_3_20.csv"
                np.savetxt(occ_file_predict_CTM, pre_occ,delimiter=',')
                flow_file = save_to+"\\KF_flow_3_20.csv"
                np.savetxt(flow_file,KF_flow,delimiter=',')
                speed_file = save_to+"\\pre_speed_3_20.csv"
                np.savetxt(speed_file,pre_speed,delimiter=',')
                speed_file = save_to+"\\KF_speed_3_20.csv"
                np.savetxt(speed_file,KF_speed,delimiter=',')                
                parameter_file = save_to+"\\parameters_3_20.csv"
